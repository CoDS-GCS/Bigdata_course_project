...
class ParallelSimilarity:
    ...
    def __get_chosen_URIs_for_relation(self, relation: str, uris: list, names: list):
        if not uris:
            return uris
        scores = self.parallel_computing_similarity(relation, names)
        l1, l2 = list(zip(*uris))
    ...
    def extract_possible_V_and_E(self, question):
        for entity in question.query_graph:
            ...
            scores = self.parallel_computing_similarity(entity, names)
        ...
        # else:
            # cprint(f"[GRAPH NODES WITH URIs:] {question.query_graph.nodes(data=True)}")
            # cprint(f"[GRAPH EDGES WITH URIs:] {question.query_graph.edges(data=True)}")
