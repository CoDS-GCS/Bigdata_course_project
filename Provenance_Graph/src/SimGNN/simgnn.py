...
class SimGNNTrainer(object):
    ...
    def process_dataset(self):
        ...
        if self.args.dataset == "ATLAS":
            root_file = self.args.dataset_path
            self.training_graphs = ATLASDataset(root_file, train=True)
            self.testing_graphs = ATLASDataset(root_file, train=False)
    ...
    def predict(self):
        ...
        self.query_graphs = ATLASDataset(self.args.dataset_path, query=True)
        
        print("Number of query graphs", len(self.query_graphs))
        self.predict_graphs = ATLASDataset(self.args.dataset_path,predict=True)
        
        #Test with only suspicious subgraph 
        self.predict_graphs = self.predict_graphs[0:39]
        
        print("Number of predict graphs", len(self.predict_graphs))
        alarms = []
        similarity_matrix = torch.full((len(self.predict_graphs), len(self.query_graphs)), float('inf'))
        for i, g1 in enumerate(self.predict_graphs):
            for j,g2 in enumerate(self.query_graphs):
                source_batch = Batch.from_data_list([g1])
                target_batch = Batch.from_data_list([g2])
                data = self.transform((source_batch, target_batch), predict=True)
                prediction = self.model(data)
                similarity_matrix[i][j] = prediction.tolist()[0]   
        
            
        if self.args.logs:
            mat_file = "./logs/" + self.args.load.split("/")[-1].replace(".pt","") +"_predicted_similarity.pt"
            ensure_dir(mat_file)
            with open(mat_file, 'wb') as f:
                pickle.dump(similarity_matrix, f)   

                
        def manually_annotated_s4_S1_queries(similarity_matrix,s1=False,s4=False):
            tp,tn,fp,fn = 0,0,0,0
            query_s1 = 0
            query_s4 = 2
            if s4:
                print("Accuracy for S4 query graph")
                for i, g1 in enumerate(self.predict_graphs):
                    if similarity_matrix[i][query_s4] > self.args.threshold:
                        if "suspicious" in g1.name and g1.has_s4 == 1: 
                            tp += 1
                        else:
                            fp += 1
                    else:
                ...
                print("Accuracy for S1 query graph")
                for i, g1 in enumerate(self.predict_graphs):
                    if similarity_matrix[i][query_s1] > self.args.threshold:
                        if "suspicious" in g1.name and g1.has_s1 == 1: 
                            tp += 1
                        else:
                            fp += 1
                    else:
            ...
            print("Accuracy: {}\tPrecision: {}\tRecall: {}\tF-1: {}".format(round(accuracy,4), round(precision,4), round(recall,4), round(f_measure,4)))

            return 
        
        
        #Revisit to check accuracy in three Threshold points
        manually_annotated_s4_S1_queries(similarity_matrix,s4=True)                
        manually_annotated_s4_S1_queries(similarity_matrix,s1=True)                
